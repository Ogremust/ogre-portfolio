...
@keyframes blob1{0%,100%{transform:translate3d(0,0,0) scale(1)}33%{transform:translate3d(80px,-110px,0) scale(1.2)}66%{transform:translate3d(-60px,70px,0) scale(.85)}}
@keyframes blob2{0%,100%{transform:translate3d(0,0,0) scale(1)}33%{transform:translate3d(-80px,90px,0) scale(.9)}66%{transform:translate3d(90px,-60px,0) scale(1.14)}}
@keyframes blob3{0%,100%{transform:translate3d(0,0,0) scale(1)}50%{transform:translate3d(-50px,-90px,0) scale(1.22)}}
@keyframes blob4{0%,100%{transform:translate3d(0,0,0) scale(1)}40%{transform:translate3d(100px,55px,0) scale(.88)}80%{transform:translate3d(-35px,-80px,0) scale(1.1)}}
@keyframes riverLeft{from{transform:translate3d(0,0,0)}to{transform:translate3d(-50%,0,0)}}
@keyframes riverRight{from{transform:translate3d(-50%,0,0)}to{transform:translate3d(0,0,0)}}
@keyframes scan{0%,100%{transform:translate3d(0,-80%,0);opacity:.14}50%{transform:translate3d(0,170%,0);opacity:.32}}
@keyframes sweep{0%{transform:translate3d(-130%,0,0) skewX(-18deg);opacity:0}30%{opacity:.85}100%{transform:translate3d(160%,0,0) skewX(-18deg);opacity:0}}
@keyframes bitePulse{0%,100%{opacity:.35;transform:scaleX(.72)}50%{opacity:1;transform:scaleX(1)}}
@keyframes modalIn{from{opacity:0;transform:translate3d(0,18px,0) scale(.98)}to{opacity:1;transform:translate3d(0,0,0) scale(1)}}
@keyframes drawerIn{from{opacity:0;transform:translate3d(0,-12px,0)}to{opacity:1;transform:translate3d(0,0,0)}}
...
@keyframes introSweep{0%{transform:translate3d(-100%,0,0) skewX(-20deg)}100%{transform:translate3d(100%,0,0) skewX(-20deg)}}
...
.tool-icon {
  ...
  box-shadow:
    inset 0 0 14px rgba(255,138,31,.55),
    inset 0 0 28px rgba(255,42,82,.25),
    0 0 16px rgba(255,138,31,.35),
    0 0 32px rgba(255,42,82,.18);
}
...
.tool-icon img {
  ...
  pointer-events: none;
  filter:
    sepia(1) saturate(5) hue-rotate(-8deg) brightness(1.4)
    drop-shadow(0 0 6px rgba(255,138,31,.9))
    drop-shadow(0 0 14px rgba(255,42,82,.65));
}
...
.glass {
  ...
  box-shadow: inset 0 1px 0 rgba(255,255,255,.22), inset 0 -1px 0 rgba(255,138,31,.18), inset 1px 0 0 rgba(255,42,82,.1), inset -1px 0 0 rgba(255,189,61,.09), 0 18px 60px rgba(0,0,0,.46);
...
.viewer{
  border-radius:22px;
  background:linear-gradient(135deg,#1b1411,#4b1718 45%,#120f0e);
  border:1px solid rgba(255,255,255,.12);
  overflow:hidden;
  /* THE FIX: Strict Flexbox to stack them separately */
  display: flex;
  flex-direction: column;
}

/* We remove the old scanner CSS here so it doesn't bleed over the UI */
.viewer::after { display: none; }

.caption{
  color:#071007;background:var(--acid);
  padding:10px 12px;border-radius:10px;text-align:center;
  font-family:"Archivo Black";text-transform:uppercase;font-size:19px;
}
...
.action-cell {
  ...
  /* THE FIX: Stops the corner bleed perfectly */
  transform: translateZ(0);
  background-clip: padding-box;
...
/* ───────────────────────────────────────────────
   REEL CARD & NATIVE IMG THUMBNAILS
─────────────────────────────────────────────── */
.reel {
  width: 198px;
  height: 352px;
...
/* The physical image tag AND inline Iframe! */
.reel-img {
  position: absolute;
  inset: 0;
  width: 100%;
  ...
  object-fit: cover;
  z-index: 0;
  /* THE STATIC OGRE COLOR GRADE */
  filter: sepia(0.5) hue-rotate(-15deg) saturate(1.8) contrast(1.1) brightness(0.85);
}
...
.reel::before {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(to top, rgba(4,4,8,0.95) 0%, transparent 60%);
  z-index: 1;
  pointer-events: none;
}
...
.reel::after {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(125deg,rgba(255,138,31,.16),transparent 42%,rgba(255,42,82,.14));
  pointer-events: none;
  z-index: 1;
}
...
/* Ensure text and buttons float above the image */
.play {
...
.social-links a {
  ...
  cursor: none; /* Keeps your custom dot cursor */
}
.social-links a:hover {
  ...
  box-shadow: 0 8px 24px rgba(255,138,31,0.12);
}
.about-photo{
...
.faq-a.open {
  grid-template-rows: 1fr; /* Expands to exact content height perfectly */
}
...
.booking-actions{grid-column:1/-1;display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:margin-top:6px}
...
.video-modal {
  ...
  box-shadow: 0 20px 80px rgba(0,0,0,.8), 0 0 0 1px rgba(255,138,31,.28);
}
.video-close {
  position: absolute;
  top: -16px;
  ...
  color: #000;
  border: none;
  box-shadow: 0 4px 12px rgba(255,138,31,.4);
  cursor: none;
}
.video-close:hover {
...
/* ───────────────────────────────────────────────
   PORTFOLIO WITH DRIVE IDS & GRADIENT MASKS
─────────────────────────────────────────────── */
const portfolio = [
  ["Reel", "Healthcare", "linear-gradient(145deg,#211412,#6b1b20 48%,#ff8a1f)", "1Du1xf2l5hxWVMKkTp_cFD3QJriAm3f_N"],
  ["Short", "Gaming 1", "linear-gradient(145deg,#1d1614,#7a2432 48%,#ffb13d)", "1ecYNq6kclM8wNZj5x9BWVCm_hZpdCjMr"],
  ["Short", "Gaming 2", "linear-gradient(145deg,#201212,#86243b 48%,#ff5c2a)", "1YmAMA93-p86qir3NAVHExTmYnZFFD1eT"],
  ["TikTok", "Trading 1", "linear-gradient(145deg,#1c1111,#5d1c24 46%,#ff2a52)", "1W6-eXqluzRQ70n78z14-bJoef8-gIRdu"],
  ["TikTok", "Trading 2", "linear-gradient(145deg,#211611,#7b3518 48%,#ffbd3d)", "1HUz1CqxddmCdV9OQ9o5W_sBlsGd5AyuE"],
  ["Reel", "Car 1", "linear-gradient(145deg,#20100f,#721b24 48%,#ff8a1f)", "1abP6iwGVlIv8tDf7goxIoI4_BwTZ34C6"],
  ["Reel", "Car 2", "linear-gradient(145deg,#191414,#6c2430 48%,#ff6a32)", "118--sTdgXkX3VbY-P4duC-AuJePoWjbg"],
  ["UGC", "Pet 1", "linear-gradient(145deg,#21130f,#7d3218 48%,#ffb13d)", "1q-MV6_hkRUOpfRRxd8nvaeRCzXiolL5W"],
  ["UGC", "Pet 2", "linear-gradient(145deg,#191112,#78202d 48%,#ff2a52)", "10CgDcVggWI8PByu63HB7HgrAf9aGcAii"],
  ["UGC", "Pet 3", "linear-gradient(145deg,#1d1210,#6f2a16 48%,#ff8a1f)", "1tflRajAz9tQMEGtNrsVQyR9afIqwGIA6"],
  ["UGC", "Pet 4", "linear-gradient(145deg,#211412,#6b1b20 48%,#ff8a1f)", "16of7CvPzohOukSzk8WWVGlDvmqHx94cZ"],
  ["UGC", "Pet 5", "linear-gradient(145deg,#1d1614,#7a2432 48%,#ffb13d)", "1neC8GvTc2xUWVf1mbcztJ2UvpHBNf3Ad"],
  ["UGC", "Pet 6", "linear-gradient(145deg,#201212,#86243b 48%,#ff5c2a)", "1WN_RPpQUk3nEg3j2V33xF7Syn-CKKB7m"],
  ["UGC", "Unboxing 1", "linear-gradient(145deg,#1c1111,#5d1c24 46%,#ff2a52)", "1sJWquYdbkJGmXUq2XD54YWwFuIRLMIIb"],
  ["UGC", "Unboxing 2", "linear-gradient(145deg,#211611,#7b3518 48%,#ffbd3d)", "1OfK2b7CpDO1g6Ih6S6mBJEykAkGlqduK"],
  ["Ad", "Product 1", "linear-gradient(145deg,#20100f,#721b24 48%,#ff8a1f)", "1jfOCH4bK6ELxEmCXCefVUuRP5AgPQ4Zq"],
  ["Ad", "Product 2", "linear-gradient(145deg,#191414,#6c2430 48%,#ff6a32)", "1Tf0fgKFL_nK8kgzENYhFba69Iq5f5vz7"],
  ["Ad", "Product 3", "linear-gradient(145deg,#21130f,#7d3218 48%,#ffb13d)", "1onLYIH8lzW6dJafdhfUmt94IbNRLC-NP"],
  ["Ad", "Product 4", "linear-gradient(145deg,#191112,#78202d 48%,#ff2a52)", "1uQ7_9nKLKgCiwz8sbddkI4u-vLTSGTnL"],
  ["Ad", "Product 5", "linear-gradient(145deg,#1d1210,#6f2a16 48%,#ff8a1f)", "1eV_OkFCt6BMtUJ4aeSLPCrHL5K6u3nHh"],
];
...
const programBadges = [
  ["Pr", prLogo, 1.25],       
  ["Ae", aeLogo, 1.43],
  ["Resolve", resolveLogo, 1.7], 
  ["Ps", psLogo, 1.4],
  ["CapCut", capcutLogo, 1.5],   
  ["Ai", aiLogo, 1.4],
  ["figma", figLogo, 1.7], 
  ["higgsfield", higLogo, 1.0],   
];
...
export default function OgrePortfolio() {
  ...
  /* ───────────────────────────────────────────────
     INLINE HERO VIDEO LOGIC
  ─────────────────────────────────────────────── */
  const localReels = ["/hero-1.mp4", "/hero-2.mp4"]; // Points directly to the public folder
  const [heroVideo, setHeroVideo] = useState(null);

  useEffect(() => {
    const randomLocal = localReels[Math.floor(Math.random() * localReels.length)];
    setHeroVideo(randomLocal);
  }, []);
  ...
  const handleTelegramSubmit = async (event) => {
    ...
    const BOT_TOKEN = "8807791744:AAEo1W5J0g7YxgHxfjR56SK8DJ02kTDeAZY";
    const CHAT_ID = "2100097919";
    
    const text = `🔥 *NEW OGRE LEAD*\n\n*Name:* ${formData.name}\n*Email:* ${formData.email}\n*Link:* ${formData.link}\n*Brief:* ${formData.brief}`;
    ...
    try {
      ...
      setFormStatus("sent");
      setTimeout(() => setBooking(false), 2000);
    } catch (error) {
      console.error(error);
      setFormStatus("idle");
    }
  ...
  useEffect(() => {
    ...
    const onMove = (event) => {
      ...
      const onButton = target instanceof Element && Boolean(target.closest("button, .reel, .viewer"));
  ...
  useEffect(() => {
    const locked = booking || intro || navOpen || activeVideo;
    const previous = document.body.style.overflow;
    if (locked) document.body.style.overflow = "hidden";
    return () => {
      document.body.style.overflow = previous;
    };
  }, [booking, intro, navOpen, activeVideo]);
  ...
  const reverseRiver = [...portfolio].reverse().concat([...portfolio].reverse());
          ...
          <div className="console-wrap">
            <aside className="console glass" aria-label="Featured reel placeholder">
              <div className="console-top">
                <span>{heroVideo ? heroVideo.replace("/","").replace(".mp4","").toUpperCase() : "OGRE_EDIT_001"}</span>
                <div className="lights"><span /><span /><span /></div>
              </div>
              <div className="viewer" style={{ cursor: 'none' }}>
                
                {/* 1. TOP BOX: Pure 1:1 Video Area */}
                <div style={{ position: 'relative', width: '100%', aspectRatio: '1 / 1', overflow: 'hidden' }}>
                  ...
                  {/* The Scanner (Confined strictly to the video) */}
                  <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '38%', background: 'linear-gradient(180deg,rgba(255,255,255,.14),transparent)', animation: 'scan 7s ease-in-out infinite', pointerEvents: 'none', zIndex: 1 }} />
                </div>

                {/* 2. BOTTOM BOX: The UI Dock (No overlap whatsoever) */}
                <div style={{ padding: '24px 18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
                  <div className="caption">Pacing that stops the scroll.</div>
        ...
        <section id="work" className="section">
          ...
          <div className="section-band">
            <div className="section-head">
              ...
              <p>Real client work. Click any reel to watch the full cut.</p>
            </div>
            <div className="portfolio-wrap">
              <div className="river">
                {river.map(([type, title, grad, driveId], i) => (
                  <article 
                    className="reel" 
                    key={`${title}-${i}`}
                    onClick={() => setActiveVideo(driveId)}
                    style={{ cursor: 'none' }} 
                  >
                    <img 
              ...
              <div className="river reverse">
                {reverseRiver.map(([type, title, grad, driveId], i) => (
                  <article 
                    className="reel" 
                    key={`${title}-r-${i}`}
                    onClick={() => setActiveVideo(driveId)}
...
<section id="about" className="section">
          ...
          <div className="about-grid">
            ...
            {/* 1. THE NEW BIO AND SOCIALS */}
            <article className="about-main glass">
              <h3>The Engine Behind OGRE.</h3>
              <p>My name is Jeric Lauresta, and I built OGRE because the editing space was getting soft. Too many decorators, not enough engineers. Coming from a background in computer engineering, I treat video editing like system architecture. I don't just "edit" videos—I reverse-engineer platform algorithms to find out exactly what makes people watch. OGRE is my specialized short-form desk. It is built on tighter pacing, deliberate visual hierarchy, and hooks that mathematically stop the scroll. You hand over the raw footage. I give you back a weapon.</p>
              
              <div className="social-links">
                {/* ⚠️ DON'T FORGET TO SWAP THESE URLS WITH YOUR ACTUAL LINKS */}
                ...
                <a href="https://www.onlinejobs.ph/jobseekers/info/4625257" target="_blank" rel="noopener noreferrer">OnlineJobs.ph</a>
              </div>
            </article>

            {/* 2. YOUR PHOTO */}
            <div className="about-photo glass">
              <img src={profilePic} alt="Jeric Lauresta" className="profile-img" loading="lazy" />
            </div>

            {/* 3. THE UPDATED TACTICAL LIST */}
            <div className="about-list">
              <div className="about-item"><strong>Style</strong><span>Clean cuts, sharp hooks, readable captions, zero filler.</span></div>
              <div className="about-item"><strong>Focus</strong><span>Direct-response UGC, viral style replication, and high-converting ads.</span></div>
              <div className="about-item"><strong>Promise</strong><span>Ruthless precision, fast delivery, and edits that actually move the needle.</span></div>
            </div>
        ...
        <section id="reviews" className="section">
          ...
          <div className="section-head">
            {/* THE FIX: nowrap forces it to one line, the clamp ensures it fits on mobile */}
            <h2 style={{ whiteSpace: 'nowrap', fontSize: 'clamp(24px, 7vw, 68px)' }}>They felt the bite.</h2>
          ...
          <div className="review-grid">
            {testimonials.map(([name, role, quote]) => (
              <article className="review-card glass" key={name}>
                ...
                {/* THE FIX: Injects 5 gold stars at the top of the card */}
                <div className="stars">
        ...
        <section id="cta" className="section">
          <div className="footer-cta glass">
            <div>
              ...
              <h2>Ready to roar?</h2>
              <p>Stop settling for average edits. One brief, one call, and your content starts working harder.</p>
            </div>
        ...
        <div className="booking-layer" role="dialog" aria-modal="true" aria-labelledby="booking-title" onClick={() => setBooking(false)}>
          <div className="booking-modal glass" onClick={(event) => event.stopPropagation()}>
            ...
            <form className="booking-form" onSubmit={handleTelegramSubmit}>
              ...
              <input 
                required 
                aria-label="Footage or reference link" 
                className="wide-field" 
                placeholder="Footage / reference link" 
                value={formData.link} 
              ...
              <textarea 
                required 
                aria-label="Brief" 
                placeholder="What are we cutting, and what should it do?" 
      ...
      ─────────────────────────────────────────────── */}
      {activeVideo && (
        <div className="booking-layer" role="dialog" aria-modal="true" onClick={() => setActiveVideo(null)}>
          <div className="video-modal" onClick={(event) => event.stopPropagation()}>
            <button className="modal-close video-close" aria-label="Close video" onClick={() => setActiveVideo(null)}>x</button>
            <iframe 
